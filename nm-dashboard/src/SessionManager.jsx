...
export default function SessionManager({ userId, socket, onStatusChange }) {
  ...
  useEffect(() => {
    ...
    socket.on('whatsapp_ready', (data) => {
      ...
      if (data.userId === userId) {
        ...
        setStatus('Connected ✅');
        setLoading(false);
        if (onStatusChange) onStatusChange(true, data.info);
      }
    });
  ...
  const handleResetSession = () => {
    if (window.confirm('Are you sure you want to reset your WhatsApp session? This will delete your current session and generate a new QR code.')) {
      setLoading(true);
      setQrCode(null);
